...
| Date | Change | Ref |
|:-----|:-------|:----|
| 2026-09-26 | `DossierSelection` no longer broadens a supplied ticket that resolves to zero eligible identities into an unrestricted search; it short-circuits to an empty selection so a missing/hidden/non-matching ticket stays a no-match. | HLD-005 |
| 2026-09-26 | `RestoreArchive` now verifies the archive before any mutation, so a tampered archive (or one the verifier already rejects) is refused with both stores untouched. It also reads and hashes every restored destination body and refuses to report success while a body does not hash to its content address, instead of only confirming a key exists. | HLD-006 |
| 2026-09-26 | Batched the `SetMemories` write-plan lookups: version targets, create-identity, subject collisions, external link endpoints and the label registry are now resolved against stored state in a handful of batched queries before the per-item loop, instead of one round-trip per item (a 200-item write went from up to ~800 to five). Conflict and not-found detection, and their order, are unchanged. | HLD-002 |
