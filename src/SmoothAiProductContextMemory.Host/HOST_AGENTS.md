...
- `GET .../blob` takes `?scope=` and `POST /query` takes `asOf`/`limit`; `PATCH /groups/{uuid}` and `GET|POST /initiatives` complete the registry surface. The blob route is a proxy **and** a scope boundary — see `Features/FEATURES_AGENTS.md` LADR-003.
