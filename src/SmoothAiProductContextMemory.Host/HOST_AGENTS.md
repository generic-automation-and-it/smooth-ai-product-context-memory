...
- Endpoints translate HTTP to Mediator only. Constraint/trigger violations map to RFC 7807 — never leak `PostgresException` or SQL.
- Scalar/OpenAPI document every `/api/context/*` route. Bind localhost (`5141`/`7141`); do not listen `0.0.0.0` in dev.
