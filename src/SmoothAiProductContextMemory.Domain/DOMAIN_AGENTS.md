...
- **Enforce invariants at construction.** Default for domain types. Guard required state in constructors/factory methods so an entity cannot exist in an invalid state. Value objects are immutable and compared by value. (Context-memory persistence rows are exempt — see the HLD 001 (storage) exceptions below.)

## HLD 001 (storage) exceptions (context-memory model)

These are deliberate departures from the defaults above, sanctioned by
[HLD 001 (storage)](../../.docs/hlds/001-context-memory-storage/). **Do not "correct" them** —
...
| Date | Change | Ref |
|:-----|:-------|:----|
| 2026-09-09 | Restored the "Enforce invariants at construction" default as the stated Domain default; the HLD 001 (storage) exceptions section now explicitly carves out the context-memory persistence rows. | HLD 001 (storage) |
| 2026-09-09 | Seven-entity context-memory model + `Slug` value logic added; recorded the HLD 001 (storage) serialization and anemic-POCO exceptions. | HLD 001 (storage) |
