...
public sealed class SamplePersistenceTests(AspireFixture aspire)
{
    ...
    [Fact]
    public async Task SmoothAiProductContextMemoryTestDatabase_CanCreateIsolatedDatabase()
    {
        await using var db = await SmoothAiProductContextMemoryTestDatabase.CreateAsync(
            aspire,
