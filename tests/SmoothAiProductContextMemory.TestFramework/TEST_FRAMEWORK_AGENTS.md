...
- **`AspireFixture`** owns L1/L2 container dependencies, provisioned by `tests/SmoothAiProductContextMemory.TestFramework.Aspire`. It resolves endpoints in three steps: (1) probe the fixed well-known ports of already-running persistent containers, (2) ask the container CLI for the published port of each `mimisbrunnr-*` container, (3) only then boot the Aspire `DistributedApplication` itself. The `DistributedApplication` is shared process-wide via `[CollectionDefinition("Aspire")]` and disposal is a deliberate no-op — containers are `ContainerLifetime.Persistent` and outlive the test run. Docker Desktop groups them under `Mímisbrunnr` (`com.docker.compose.project`); container names are `mimisbrunnr-{tech}` (ASCII — Docker names cannot carry the acute).
...
- **Test dependency ports** (fixed, must never collide with the dev AppHost's `smooth-project-memory-dev-*` resources):
  ...
  |---|---|---:|
  | PostgreSQL | `mimisbrunnr-postgres` (`docker.io/apache/age:release_PG17_1.7.0`) | `15432` |
  | Redis | `mimisbrunnr-redis` | `16379` |
  | WireMock | `mimisbrunnr-wiremock` | `19091` |
  | MinIO (blob) | `mimisbrunnr-blob` | `9002` (s3), `19092` (console) |
...
| Date | Change | Ref |
|:-----|:-------|:----|
| 2026-09-13 | Test containers named `mimisbrunnr-{tech}`; Docker Desktop group `Mímisbrunnr`. Recreate persistent `project-test-*` containers once so Aspire does not keep the old names. | — |
| 2026-09-13 | Test Postgres image pinned to `docker.io/apache/age:release_PG17_1.7.0`. Recreate `mimisbrunnr-postgres` once after the pin — `ContainerLifetime.Persistent` keeps the previous image until the container is removed. | HLD-003 |
