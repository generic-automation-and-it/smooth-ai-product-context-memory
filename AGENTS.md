...
SMOOTH_SNAPSHOT_BENCH=1 dotnet test tests/SmoothAiProductContextMemory.Infrastructure.ComponentTest \
    --filter SnapshotEvidenceTests                                 # HLD-006 NFR-04 snapshot timing evidence (skipped without the env var)
...
| Date | Change | Ref |
|---|---|---|
| 2026-09-25 | Simplified the previous row's `pipeline-ai-analyse.yml` "Commit fixes" fix after checking the upstream `smooth-ai-report-review`'s equivalent step, which never hit this bug: the `$RUNNER_TEMP` move/restore dance was unnecessary. Empirically confirmed (`git add -A -- .` vs `git add -A -- . ':(exclude,glob)ci_temp/**'` against a scratch repo) that a bare `git add -A -- .` with **no** exclude pathspec silently skips a gitignored directory — the exit-1 only happens when the ignored path is *named* in a pathspec at all (exclude or otherwise), which is exactly what upstream's own comment in that step already warns against. Since `ci_temp`, `.context` and `.smooth-ai-review-tools` are all gitignored here, the step now drops the pathspec entirely (`git status --porcelain -- .` / `git add -A -- .`) instead of moving `ci_temp` out of the tree. Same conclusion, cheaper mechanism; upstream needs no change because its own step never named `ci_temp` in a pathspec. | `.github/workflows/pipeline-ai-analyse.yml` |
| 2026-09-25 | Fixed `pipeline-ai-analyse.yml`'s "Commit fixes" step, which failed (run 36099261149) the first time the fixer actually produced edits: `git add -A -- . ':(exclude,glob)ci_temp/**'` still refuses (exit 1) the instant it walks into the gitignored `ci_temp` directory *entry itself* while recursing `.` — the exclude pathspec only keeps its contents out of the index, not the directory node, and no pathspec form (literal exclude, `--ignore-errors`, `advice.addIgnoredFile=false`) works around it; only removing the ignored directory from the tree does. The step now moves `ci_temp` to `$RUNNER_TEMP` immediately before `git add -A -- .` and moves it straight back afterward, so the later "Post ai-analyse summary" step can still read `ci_temp/analyse_out.md`. Root cause traces to the 2026-09-24 `.gitignore` row below, which made `ci_temp` bare-name-ignored without re-checking this step's `git add`. Superseded by the row above, which found a simpler fix. | run 36099261149, `.github/workflows/pipeline-ai-analyse.yml` |
...
| 2026-09-25 | Added L1 coverage for HLD-006: a snapshot→restore round-trip test (reconciliation closes, `TICKET_PARENT` direction preserved), a missing-blob negative control, and an empty-refusal test, plus the env-gated `SnapshotEvidenceTests` harness recording the NFR-04 snapshot timing (2.80 s at 1,000 memories/1,000 bodies) as `nfrs/NFR-04-snapshot-measurement.md`. This surfaced and fixed two restore bugs: invalid vertex `MERGE (n:Memory) {props}` syntax (props must sit inside the node parens) and misplaced interpolation braces in the edge Cypher. | HLD-006 |
...
| 2026-09-21 | Completed the review-gate `main`-tracking realignment from PR #95: the four live prose spots the merged diff missed — the gate's header comment (`pipeline-code-review-report.yml`), the analyse workflow's "pins by SHA" comment, the `AGENTS.md` CI/CD bullet, and the `docs/wiki/ci.md` Packaging bullet — now describe the upstream checkout as tracking `main` rather than as a pinned SHA. No behavioural change. | PR #95 review |
| 2026-09-21 | Review-gate upstream checkout switched from a pinned SHA to tracking `main`: `.github/workflows/pipeline-code-review-report.yml` `ref:` is now `main`, since the consumer owns `generic-automation-and-it/smooth-ai-report-review` (same org — the third-party supply-chain concern that motivates SHA-pinning does not apply) and forking is not in play. Comments realigned to the `main`-tracking rationale, brought in step with `pipeline-ai-analyse.yml` (which already tracked `main`); `docs/wiki/ci.md` and `pr-standards.instructions.md` pin-reference prose and the round-trip fetch switched to `ref=main`. Must re-verify `main` still ships `run-review.sh` and stays on the OpenCode v2 line on upstream drift. | `.github/workflows/pipeline-code-review-report.yml`, [ci.md](docs/wiki/ci.md) |
| 2026-09-21 | AI PR review synced to the upstream OpenCode v2 line: `.github/workflows/pipeline-code-review-report.yml` bumped its pinned upstream SHA to the merged v2 migration commit `7cc2d09` (was the pre-v2 `4bdfea4`, whose v1 scripts failed against a 2.x `OPENCODE_CLI_VERSION` as `gzip: stdin: not in gzip format`); `pipeline-ai-analyse.yml` gained the v2 `opencode service stop` + `opencode-health.sh` exit-3 abort in its Initialize step. `docs/wiki/ci.md` and `pr-standards.instructions.md` pin references realigned to `7cc2d09`. Consumer provider-id ladder still has no `OPENCODE-GO-RESPONSES` row. | `.github/workflows/pipeline-code-review-report.yml`, [ci.md](docs/wiki/ci.md) |
...
| 2026-09-19 | Retargeted the gate's unset-Variable fallbacks from `ANTHROPIC`/`claude-opus-5` to `OPENCODE-GO-OPENAI`/`glm-5.2`, closing the question the row above left open — an unconfigured consumer now lands on the provider the gate actually resolves. Six coupled literals plus two comments moved together: selector, provider-id bare literal, terminator guard, and three model tiers. The guard now correctly does **not** start the terminator on an unset Variable, and `ANTHROPIC` keeps its explicit provider-id row so selecting it still works. | PR #83 review |
