...
- Review whether any HLD under `docs/hlds/` (including its `nfrs/` and ADR pointer stubs) needs updating — if the implementation changes behavior covered by an existing HLD or NFR, update it; if a new architectural decision was made, author a new HLD via the create-hld skill
