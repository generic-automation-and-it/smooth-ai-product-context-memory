...
- Use domain and pattern vocabulary the next engineer already knows (`MemoryVersion`, `ExportStore`).
...
- **No comments unless the user explicitly asks.** Do not add one to justify code that is hard to
  read — rename or extract instead.
...
- **Objects hide their internals and expose behaviour; data structures expose their fields and carry no
  ...
  ID) is an object, not a hybrid. Context-memory persistence rows are anemic POCOs by HLD 001 — do
