...
- Where a principle conflicts with an ADR, the **ADR wins** — say so, do not silently comply. A
  horizontal `*Service` layer is the standard SOLID-trained instinct;
  [`backend/architecture-slices`](backend/architecture-slices.instructions.md) forbids it — use
  `Features/<Name>/` handlers, not a `Commands/` / `Queries/` / `Services/` split.
...
| Date | Change |
|:-----|:-------|
| 2026-09-13 | Copied from gf-dynamic-pallet-position; Service-layer ADR retargeted to architecture-slices. |
