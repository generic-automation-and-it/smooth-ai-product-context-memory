...
- **Never return descriptive fields from a read path without the scope plan.** A traversal that returns `statement` or `description` is a read path, so `MemoryScopeFilter.Plan` goes into its SQL. Omitting it reopens the hole the blob proxy once had.
