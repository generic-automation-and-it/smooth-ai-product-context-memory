...
- **Never return descriptive fields from a read path without the scope plan — and a path's intermediate hops are part of that read.** `MemoryScopeFilter.Plan` goes into the traversal SQL for the endpoint *and* for every vertex the path crosses. Gating only the endpoint still discloses a hidden memory's identity and its edges' reasons; that was found in review, reproduced by a test, and fixed. Omitting either reopens the hole the blob proxy once had.
...
- **The intermediate-hop scope gate is a membership test, not a join.** Joining `memory` inside the `NOT EXISTS` made the planner hash all 3,000 rows per candidate path — it estimates 100 rows from `jsonb_array_elements` and cannot know a bounded path holds four — which doubled the composed shape to 16.1 ms. Testing membership against the set of memories in excluded scopes, computed once, costs 0.15 ms.
