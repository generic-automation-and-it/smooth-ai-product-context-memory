...
- **BR-46 is a narrow proposed exception, not a repeal of the historical-ingestion exclusion.** It permits an optional initial baseline from one practitioner-chosen project, with an optional next-feature focus, reviewed selection and visible recall coverage. It does not permit bulk back-filling or recurring repository ingestion; BR-01 still governs ongoing capture.
...
| 2026-09-20 | Local amendment proposal: retained the general historical-ingestion exclusion but added BR-46's narrow proposed exception for a bounded, practitioner-reviewed initial project bootstrap with cited candidates, consequential questions, preserved conflicts/lifecycle, duplicate-safe reruns and visible recall coverage. Pending upstream acceptance. | BR-46 |
